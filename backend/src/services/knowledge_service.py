...
class KnowledgeService:
    ...
    async def _check_category_name_unique(self, name: str, parent_id: str = None) -> None:
        """检查目录名称唯一"""
        stmt = select(KnowledgeCategoryModel).where(
            and_(
                KnowledgeCategoryModel.name == name,
                KnowledgeCategoryModel.parent_id == parent_id
            )
        )
    ...
    async def update_category(self, category_id: str, name: str = None, parent_id: str = None) -> dict:
        """更新目录"""
        stmt = select(KnowledgeCategoryModel).where(KnowledgeCategoryModel.id == category_id)
        result = await self.db.execute(stmt)
    ...
    async def delete_category(self, category_id: str) -> bool:
        """删除目录"""
        # 检查子目录
        stmt = select(KnowledgeCategoryModel).where(KnowledgeCategoryModel.parent_id == category_id)
        result = await self.db.execute(stmt)
        if result.scalar_one_or_none():
            raise ValueError("请先删除子目录")

        # 检查文件
        stmt = select(KnowledgeDocumentModel).where(KnowledgeDocumentModel.category_id == category_id)
        result = await self.db.execute(stmt)
        if result.scalar_one_or_none():
    ...
    async def get_category_tree(self) -> list:
        """获取目录树"""
        stmt = select(KnowledgeCategoryModel).order_by(
            KnowledgeCategoryModel.order,
            KnowledgeCategoryModel.created_at
        )
        result = await self.db.execute(stmt)
    ...
    def _category_to_dict(self, category: KnowledgeCategoryModel) -> dict:
        """模型转字典"""
        return {
            "id": category.id,
            "name": category.name,
    ...
    async def get_documents(self, category_id: str = None) -> list:
        ...
        if category_id:
            stmt = stmt.where(KnowledgeDocumentModel.category_id == category_id)
    ...
    def _document_to_dict(self, document: KnowledgeDocumentModel) -> dict:
        """模型转字典"""
        return {
            "id": document.id,
