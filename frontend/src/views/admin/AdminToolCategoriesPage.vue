<template>
  <div class="admin-tool-categories-page">
    <el-card>
      <template #header>
...
const form = reactive<(CreateToolCategoryRequest | UpdateToolCategoryRequest) & { id?: string }>({
  name: '',
  icon: '',
  order: 0,
})
 ...
 * 编辑分类
 */
function handleEdit(category: AdminToolCategoryListItem) {
  Object.assign(form, {
    id: category.id,
    name: category.name,
    icon: category.icon || '',
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    if (!form.icon || form.icon.trim() === '') {
      form.icon = recommendIcon(form.name || '', '', 'folder')
    }
    ...
    try {
      if (isEdit.value && form.id) {
        ...
        await ApiService.updateToolCategory(id!, requestData)
        ElMessage.success('更新分类成功')
      } else {
        await ApiService.createToolCategory(form as CreateToolCategoryRequest)
 ...
 * 删除分类
 */
async function handleDelete(category: AdminToolCategoryListItem) {
  ...
  try {
    ...
    loading.value = true
    try {
      await ApiService.deleteToolCategory(category.id)
 ...
 * 上移分类
 */
async function handleMoveUp(category: AdminToolCategoryListItem) {
  try {
    await ApiService.moveCategoryUp(category.id)
 ...
 * 下移分类
 */
async function handleMoveDown(category: AdminToolCategoryListItem) {
  try {
    await ApiService.moveCategoryDown(category.id)
