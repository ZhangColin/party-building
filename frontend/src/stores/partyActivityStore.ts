...
export const usePartyActivityStore = defineStore('partyActivity', () => {
  // ==================== 状态 ====================
  ...
  async function loadCategoryTree() {
    loading.value = true
    try {
      categories.value = await partyActivityApi.getCategoryTree()
    } finally {
      loading.value = false
    }
  }
  ...
  async function createCategory(data: CategoryCreateRequest) {
    const category = await partyActivityApi.createCategory(data)
    await loadCategoryTree()
    return category
  }

  async function updateCategory(categoryId: string, data: CategoryUpdateRequest) {
    await partyActivityApi.updateCategory(categoryId, data)
    await loadCategoryTree()
  }

  async function deleteCategory(categoryId: string) {
    await partyActivityApi.deleteCategory(categoryId)
  ...
  async function loadDocuments(categoryId?: string) {
    loading.value = true
    try {
      documents.value = await partyActivityApi.getDocuments(categoryId)
    } finally {
      loading.value = false
    }
  }
  ...
  async function createDocument(data: DocumentCreateRequest) {
    const document = await partyActivityApi.createDocument(data)
  ...
  async function uploadFile(
    ...
    const result = await partyActivityApi.uploadFile(file, categoryId, onProgress)
    await loadDocuments(categoryId)
    return result
  }

  async function loadDocument(documentId: string) {
    return await partyActivityApi.getDocument(documentId)
  }

  async function updateDocument(documentId: string, content: string) {
    const document = await partyActivityApi.updateDocument(documentId, { content })
  ...
  async function deleteDocument(documentId: string) {
    await partyActivityApi.deleteDocument(documentId)
    documents.value = documents.value.filter(d => d.id !== documentId)
    selectedDocuments.value = selectedDocuments.value.filter(id => id !== documentId)
  }

  async function downloadDocument(documentId: string) {
    return await partyActivityApi.downloadDocument(documentId)
  }
